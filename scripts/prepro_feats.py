...
def oldMain(imgs, params, iStart, iEnd, id_):
    ...
    for i,img in enumerate(imgs[iStart:iEnd]):
        # load the image
        ...
        if i % 1000 == 0:
            print('process %d: %d/%d (%.2f%% done)' % (id_, i+iStart, iEnd-iStart, (i*100.0)/(iEnd-iStart)))
...
def main(params):
    ...
    for i in range(numThreads):
        ...
        if i==numThreads:
            ...
            iEnd=(i+1)*groupSize
        p = mp.Process(target=oldMain, args=(imgs, params, iStart, iEnd, i))
        p.start()
        threads.append(p)
